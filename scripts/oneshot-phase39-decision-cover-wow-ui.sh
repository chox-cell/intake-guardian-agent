...
ROOT="$(pwd)"
echo "==> Phase39 OneShot (Decision Cover™ WOW UI + demo generator + smoke) @ $ROOT"

[ -d "src" ] || { echo "ERROR: run inside repo root (src missing)"; exit 1; }
[ -d "scripts" ] || { echo "ERROR: scripts missing"; exit 1; }

STAMP="$(date +%Y%m%d_%H%M%S)"
BK="__bak_phase39_${STAMP}"
mkdir -p "$BK"
cp -R src scripts docs package.json tsconfig.json "$BK" 2>/dev/null || true
echo "✅ backup -> $BK"

mkdir -p src/ui

# -----------------------------
# [1] WOW Decisions UI Route (safe overwrite)
# -----------------------------
cat > src/ui/decisions_route.ts <<'TS'
...
async function createDemoDecision(tenantId: string) {
  ...
  const actions =
    tier === "GREEN"
      ? ["Proceed with confidence", "Send client Proof ZIP", "Archive decision"]
      : tier === "AMBER"
  ...
  await store.add({
    id,
    createdAt: now,
    title: "Decision Cover — Demo Decision",
    tier,
    score,
    reason:
      tier === "GREEN"
...
export function mountDecisionsUI(app: Express) {
  ...
  app.get("/ui/decisions", async (req: Request, res: Response) => {
    ...
    const hero = `
      <div class="hero">
        <div>
          <div class="h1">Decision Cover™ — Decisions</div>
          <div class="muted">If you must decide, decide with proof. This page shows every decision generated from your intake.</div>
        </div>
        <div class="row">
          <a class="btn" href="${links.demo}">⚡ Generate demo decision</a>
          <a class="btn" href="${links.setup}">🧭 Setup</a>
          <a class="btn" href="${links.tickets}">🎟 Tickets</a>
          <a class="btn" href="${links.csv}">⬇ Export CSV</a>
          <a class="btn" href="${links.zip}">📦 Evidence ZIP</a>
        </div>
      </div>
      <div class="row">
        <div class="pill">Tenant: <span class="k">${esc(p.tenantId)}</span></div>
        <div class="pill">Key: <span class="k">${esc(p.k.slice(0, 8))}…</span></div>
      </div>
    `;

    const cards =
      rows.length === 0
        ? `<div class="card"><div class="big">No decisions yet.</div><div class="muted">Click “Generate demo decision” to see how the product looks for a client.</div></div>`
        : `<div class="grid">${rows
            .map((r) => {
    ...
    const body = `${hero}${cards}<div class="footer">Decision Cover™ demo UI. Next: connect real pack ZIP per decision (Phase40).</div>`;
    res.setHeader("content-type", "text/html; charset=utf-8");
    res.status(200).send(layout("Decision Cover — Decisions", body));
  });
...
echo "✅ wrote src/ui/decisions_route.ts (WOW UI + demo generator)"

# -----------------------------
# [2] Smoke Phase39
# -----------------------------
cat > scripts/smoke-phase39.sh <<'BASH'
#!/usr/bin/env bash
set -euo pipefail
