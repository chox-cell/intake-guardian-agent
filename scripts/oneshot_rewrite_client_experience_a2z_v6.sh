...
cat > scripts/client_experience_a2z.sh <<'BASH'
#!/usr/bin/env bash
set -euo pipefail
...
VERIFY_URL="$(rg -n 'http://127\.0\.0\.1:7090/api/auth/verify\?token=[A-Za-z0-9_-]+' "$MAIL" -o | head -n 1 || true)"
if [ -z "${VERIFY_URL:-}" ]; then
  # fallback: any /api/auth/verify link
  VERIFY_URL="$(rg -n '/api/auth/verify\?token=[A-Za-z0-9_-]+' "$MAIL" -o | head -n 1 || true)"
...
echo "OK ✅ Rewrote scripts/client_experience_a2z.sh (Node-only, no python, clean)."
echo "Backup: $BAK"
