...
export function makeRoutes(args: {
  ...
  r.get("/health", async (_req, res) => res.json({ ok: true }));
  ...
  r.get("/workitems", async (req, res) => {
    ...
    const status = req.query.status ? z.enum(["new","triage","in_progress","waiting","resolved","closed"]).parse(req.query.status) : undefined;
    ...
    const offset = req.query.offset ? z.coerce.number().int().min(0).parse(req.query.offset) : 0;
  ...
  r.post("/workitems/:id/status", async (req, res) => {
    ...
    if (!out.ok && out.error === "not_found") return res.status(404).json(out);
  ...
  r.get("/workitems/:id/events", async (req, res) => {
    const tenantId = z.string().min(1).parse(req.query.tenantId);
    const limit = req.query.limit ? z.coerce.number().int().min(1).max(1000).parse(req.query.limit) : 200;
