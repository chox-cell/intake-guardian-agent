...
try {
  ...
  r.get("/export.csv", async (req: Request, res: Response) => {
    ...
    const lines = [
      ...
      ...(items || []).map((it: any) => {
        const id = (it.id || it.ticketId || it.workItemId || "");
        const subject = (it.subject || it.title || "");
        const from = (it.from || it.sender || "");
        const status = (it.status || "new");
        const priority = (it.priority || "normal");
        const due = (it.dueAt || it.due || it.slaDue || it.sla || "");
        return `${csvEscape(id)},${csvEscape(subject)},${csvEscape(from)},${csvEscape(status)},${csvEscape(priority)},${csvEscape(due)}`;
      })
    ].join("\n");
